...
- **index.html ("Elif Kaya", gerçek müşteri satış ajanı) — HÂLÂ BAĞLANMADI.** Bu aslında şemanın asıl hedef kullanım senaryosu (gerçek müşteri/lead konuşmaları) ama ÇALIŞAN prodüksiyon ajanını riske atmamak için portal.html'de altyapı kanıtlandıktan sonra taşınması planlanıyor — bir sonraki adım budur.
