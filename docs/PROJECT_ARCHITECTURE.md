...
**2026-08-27 güncellemesi**: `customers`, `customer_interests`, `conversations`, `conversation_messages`, `conversation_summaries` tabloları eklendi (bkz. `worker-portal/migrations/0001_conversation_memory.sql` + DATABASE_SCHEMA.md) ve bunlara karşılık gelen API uçları (`/api/customers*`, `/api/conversations*`) yazıldı, test edildi (14 yeni test — tenant izolasyonu dahil, `worker-portal/test/`). **Ama hâlâ gerçek, dürüst bir eksik var**: `agent-core/orchestrator.js` bu yeni uçları HENÜZ ÇAĞIRMIYOR — yani canlı bir görüşme sırasında transkript/özet OTOMATİK olarak bu tablolara yazılmıyor, API sadece hazır ve test edilmiş durumda. Konuşma geçmişi hâlâ yalnızca tarayıcı belleğinde (`this.history`) tutuluyor, sayfa kapanınca kayboluyor. Bir sonraki adım: `orchestrator.js`'in `onTranscript` callback'ini (veya widget-runtime.js'i) bu yeni `/api/conversations/*` uçlarına bağlamak — index.html'in ÇALIŞAN müşteri ajanını bozma riskini azaltmak için önce admin/portal widget'larında denenip sonra index.html'e taşınması planlanıyor.
